...
both the nodal kernel and this postprocessor.
...
where $C_n$ is the concentration of clusters of size $n$, $N$ is the total number of cluster sizes, and $\Omega_a$ is the atomic volume. By default $n_\text{min} = 1$, which includes monomers.  $\rho$ is the average over all nodes in the domain or subdomain. When [!param](/Postprocessors/ClusterTotalDensity/atomic_volume) is provided in m$^3$, the output is reported in `#/m^3`.
