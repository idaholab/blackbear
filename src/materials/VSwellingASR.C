...
VSwellingASR::validParams()
{
  ...
  params.addParam<MooseEnum>(
      "ASR_formulation", ASR_formulation, "isotropic or anisotropic swelling, default: isotropic");
  ...
  params.addParam<Real>(
      "alpha", 1.3333, "ASR latency time retardation constant under hydrostatic compression");
...
VSwellingASR::applyASRStrain(unsigned qp, const Real v0OverVOld, SymmTensor & strain_increment)
{
  ...
  // stress dependant total ASR volumetric accounting for ASR gel adsorption due to tensile and
  ...
  Real inc_ASR_volumetric_strain = gel_adsorption_tensile * gel_adsorption_compress *
                                   std::pow(rh_value, _m_power) * inc_ASR_extent *
                                   _ASR_final_vstrain;
  ...
  switch (_ASR_formulation)
  {
    ...
    case 1: // anisotropic swelling
    {
      ...
      SolidModel::rotateSymmetricTensor(
          evec, _inc_weighted_ASR_strain_principal, _inc_weighted_ASR_strain_cartesian);
    ...
    default:
      mooseError("Unknown ASR swelling model");
      break;
  }
...
VSwellingASR::computeDerivative(unsigned qp, Real scalar)
{
  ...
  Real jac = 1.0 - _dt / 86400.0 / tau_c * (1 - scalar) / (1 + std::exp(-tau_L / tau_c)) -
             _dt / 86400.0 / tau_c * (-1) / (1 + std::exp(-tau_L / tau_c)) *
                 (scalar + std::exp(-tau_L / tau_c));
  ;
  return jac;
}
