...
ConcreteElasticASR::validParams()
{
  ...
  params.addParam<Real>(
      "tau_c_T0", 50.0, "characteristic ASR time (in days) at reference temperature");
  ...
  params.addParam<Real>(
      "relative_tolerance", 1e-5, "Relative convergence tolerance for sub-newton iteration");
  params.addParam<Real>(
      "absolute_tolerance", 1e-20, "Absolute convergence tolerance for sub-newton iteration");

  return params;
}

ConcreteElasticASR::ConcreteElasticASR(const InputParameters & parameters) : SolidModel(parameters)
{
