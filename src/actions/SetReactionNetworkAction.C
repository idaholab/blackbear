...
SetReactionNetworkAction::validParams()
{
  ...
  params.addRequiredParam<std::vector<Real>>("initial_condition",
                                             "initial values for the primary aqueous sepcies");

  params.addParam<std::vector<AuxVariableName>>("secondary_aqueous_species",
                                                "The list of secondary aquesous species to add");
  params.addParam<std::string>("aqueous_speciations", "The list of aqueous equilibrium reactions");

  params.addParam<std::vector<AuxVariableName>>(
      "minerals", "The list of minerals adding to the aux variables ");
  params.addParam<std::vector<Real>>("initial_mineral_conc",
                                     "initial values for mineralsin  mol/L of solution ");

  params.addParam<std::vector<std::string>>("mineral_solution_reactions",
                                            "The list of mineral-solution reactions");
  params.addParam<std::vector<Real>>("log_k",
  ...
  params.addParam<Real>("system_temperature",
  ...
  params.addParam<MooseEnum>(
      "family",
      familyEnum,
      "Specifies the family of FE shape functions to use for the q AuxVariables");
  MooseEnum orderEnum = AddAuxVariableAction::getAuxVariableOrders();
  params.addParam<MooseEnum>(
      "order",
      orderEnum,
      "Specifies the order of the FE shape function to use for the q AuxVariables");
...
SetReactionNetworkAction::act()
{
  ...
  std::vector<NonlinearVariableName> nl_vars =
      getParam<std::vector<NonlinearVariableName>>("primary_aqueous_species");
  std::vector<Real> initial_vals = getParam<std::vector<Real>>("initial_condition");
  std::vector<AuxVariableName> aux_vars =
      getParam<std::vector<AuxVariableName>>("secondary_aqueous_species");
  ...
  else if (_current_task == "add_primary_species_kernels")
  {
    ...
    for (unsigned int i = 0; i < nl_vars.size(); i++)
    {
      ...
      InputParameters params_euler =
          _factory.getValidParams("PrimaryAqueousSpeciesTimeIntegration");
      params_euler.set<NonlinearVariableName>("variable") = nl_vars[i];
      params_euler.set<MaterialPropertyName>("property_name") = "porosity";
      _problem->addKernel(
          "PrimaryAqueousSpeciesTimeIntegration", nl_vars[i] + "_timeintegration", params_euler);
  ...
  */
  else if (_current_task == "add_secondary_species_kernels")
  {
    if (_pars.isParamValid("secondary_aqueous_species"))
    {
      ...
      std::vector<std::vector<std::string>>
          primary_aqueous_species_involved; // per aqueous speciation reaction
      ...
      while (re_reactions.FindAndConsume(&input, &single_reaction, &equal_coeff))
      {
        ...
        // mooseAssert(stos[n_reactions-1].size() ==
        // primary_aqueous_species_involved[n_reactions-1], "aqueous parser not works ");
        ...
        for (unsigned int i = 0; i < stos[n_reactions - 1].size(); ++i)
        {
          if (i < stos[n_reactions - 1].size() - 1)
            _console << "(" << stos[n_reactions - 1][i] << ")"
                     << primary_aqueous_species_involved[n_reactions - 1][i] << " + ";
          else
            _console << "(" << stos[n_reactions - 1][i] << ")"
                     << primary_aqueous_species_involved[n_reactions - 1][i] << "   ";
        }
      ...
      for (unsigned int i = 0; i < nl_vars.size(); ++i)
      {
        ...
        for (unsigned int j = 0; j < eq_const.size(); j++)
          if (primary_participation[i][j])
          {
            ...
            _problem->addKernel("SecondaryAqueousSpeciesTimeIntegration",
                                nl_vars[i] + "_" + eq_species[j] + "_sub",
                                params_sub);
            ...
            _problem->addKernel("SecondaryAqueousSpeciesDiffusion",
                                nl_vars[i] + "_" + eq_species[j] + "_cd",
                                params_cd);
          }
      }
    }
  ...
  else if (_current_task == "add_minerals_initial_conc")
  {
    ...
    mooseAssert(initial_mineral_vals.size() == mineral_vars.size(),
                "expected intitial values assigned to each mineral");
    _console << std::endl;
    _console << "*********************************************************************************"
             << std::endl;
  ...
  else if (_current_task == "add_minerals_kernels")
  {
    if (_pars.isParamValid("minerals"))
    {
      ...
      std::vector<std::string> mineral_reactions =
          getParam<std::vector<std::string>>("mineral_solution_reactions");
      if (aux_vars.size() == 0)
        mooseWarning("No equilibrium aqueous species provided!");
      ...
      for (unsigned int i = 0; i < nl_vars.size(); ++i)
      {
        ...
        for (unsigned int j = 0; j < mineral_reactions.size(); ++j)
        {
          ...
          if (primary_participation[j])
          {
            ...
            _console << "Weight of mineral " << coupled_var[0]
                     << " to primary sepcies " + nl_vars[i] << " = " << mineral_weight[0] << "\n";
            ...
            _problem->addKernel("MineralSolutionTimeIntegration",
                                nl_vars[i] + "_" + solid_kin_species[j] + "_kin",
                                params_kin);
          }
        }
      }
    ...
    if (_pars.isParamValid("minerals"))
    {
      ...
      for (unsigned int j = 0; j < mineral_reactions.size(); j++)
      {
        ...
        _problem->addAuxKernel(
            "MineralDissolutionPrecipAux", "aux_" + solid_kin_species[j], params_kin);
