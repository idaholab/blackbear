...
SS316ROMStressPrediction::computeResidual(const Real effective_trial_stress, const Real scalar)
{
  ...
  const Real effective_strain_old = std::sqrt(2.0 / 3.0 * _mechanical_strain_old[_qp].doubleContraction(_mechanical_strain_old[_qp]));
  ...
  std::cout << "  input vonmises devitoric stress: " << effective_trial_stress << "\n";
  std::cout << "  value 3G: " << _three_shear_modulus * scalar << "\n";
  std::cout << "  trial stress into rom: " << trial_stress_mpa << "\n";
  ...
  std::cout << "  rom_effective_strain: " << rom_effective_strain <<"\n";
  std::cout << "  mobile disl increment: " << _mobile_dislocation_increment[_qp] << "\n";
  std::cout << "  immobile disl increment: " << _immobile_dislocation_increment[_qp] << "\n";
...
{
  const Real trial_stress_mpa = (effective_trial_stress - _three_shear_modulus * scalar) * 1.0e-6;
  // const Real trial_stress_mpa = effective_trial_stress * 1.0e-6;
  const Real effective_strain_old = std::sqrt(2.0 / 3.0 * _mechanical_strain_old[_qp].doubleContraction(_mechanical_strain_old[_qp]));
